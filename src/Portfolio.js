...
const WebPort = () => {
  ...
  const land = useMediaQuery({
    //Ipad
    query:
      "(min-width : 481px) and (max-device-width : 800px) and (orientation : landscape)",
  });
  ...
  return (
    <div className="flex jc-flex al-flex h-100 pos-rel mt-5" id="websites">
      <div className="flex flx-wrp h-vh" id="web-scrll">
        {data.map((elem, ndx) => (
          <div
            className={`flex al-flex jc-flex flx-col flx-shr mt-5 w-50 fadeInDown`}
            key={ndx}
          >
            <div className="p-20 cust br-15 bg-toL flex jc-flex al-flex popShdw webItem">
              <img
                src={elem.src}
                alt={elem.alt}
                className="w-80 h-per img-shdw"
              />
            </div>
            <p
              className="glassBtn popShdw clr-wht al-ce lato p-10 mt-2 w-50 cursor hov bold pos-rel webBtn"
...
export const DesPort = (prop) => {
  ...
  return (
    <section
      id="desPort"
      className="fade ml-10 mr-10 h-100 flx-col p-20 mid over-hide flow-y-scr hide-bar popShdw trans fadeInDown"
      id="design"
    >
      <div className="flex flx-col al-flex jc-flex w-100 h-auto" id="des-main">
        <div className="flex jc-flex w-100" id="menuFilt">
          <div
            className="flex w-100 jc-flex mt-2 flx-col filter bg-toR shadow glass p-20"
            onClick={() => filter("shirt")}
          >
            <FontAwesomeIcon
              className="tech1 clr-wht"
              icon="tshirt"
              onClick={() => filter("shirt")}
            />
            <p
              className="al-ce ml-2 mt-2 pos-rel mont clr-wht bold"
              onClick={() => filter("shirt")}
            >
              Shirts
            </p>
