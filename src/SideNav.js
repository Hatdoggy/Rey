...
export const Header = (prop)=>{
	...
	mobile?
		<header className="flex al-flex bg-white p-10 shadow front-1 fadeInDown pos-stk top-0">
			...
			<FontAwesomeIcon icon="bars" className="tech ml-2" onClick={()=>prop.menu(true)}/>

	</header>	
	:<header className="flex jc-flex al-flex w-100 pos-fix shadow front-1 bg-white p-10 fadeInDown top-0">
		<Router>
		<h4 className="lato cursor vio"><Scroll to="main-top" spy={true} smooth={true}>Top</Scroll></h4>
		<h4 className="lato cursor vio ml-2"><Link onClick={()=>prop.ret("/")} replace to='/'>About me</Link></h4>
		<Logo/>
		<h4 className="lato cursor vio ml-2"><Link onClick={()=>prop.ret("/proj")} replace to='/proj'>Projects</Link></h4>
		<h4 className="lato cursor vio ml-2"><Link onClick={()=>prop.ret("/cont")} replace to='/cont'>Contact me</Link></h4>	
		</Router>		
	</header>
		);
	...
	return(
		<nav className="w-100 flex jc-flex al-flex h-100 frnt-top bg-white pos-fix fade_5">
			...
			<div className="flex flx-col jc-flex h-100 w-100">
				<Router>
				<h4 className="lato bold cursor vio mt-5 al-ce"><Link onClick={()=>prop.ret("About")} replace to='/'>About me</Link></h4>
				<h4 className="lato bold cursor vio mt-5 al-ce"><Link onClick={()=>prop.ret("Projects")} replace to='/proj'>Projects</Link></h4>
				<h4 className="lato bold cursor vio mt-5 al-ce"><Link onClick={()=>prop.ret("Contact")} replace to='/cont'>Contact me</Link></h4>	
